...
class Create extends Component
{
    ...
    protected $rules = [
        'lawyer.name' => 'required|unique:lawyers,name,null|string|min:3|max:191',
    ...
    public function save()
    {
        ...
        $lawyer = Lawyer::create($this->lawyer->toArray());
        //$this->lawyer->save();
        $lawyer->languages()->sync($this->sel_languages);
        $lawyer->cities()->sync($this->sel_cities);
        $lawyer->areas()->sync($this->sel_areas);
        $lawyer->graduation()->sync($this->sel_graduation);
        $lawyer->experience()->sync($this->sel_experience);
        $lawyer->awards()->sync($this->selAwards);
        $this->emitUp('lawyerUpdated'); //atualiza lawyer
        $this->open = false;
    }

    public function render()
    {
        return view('livewire.lawyer.create');
    }
}
