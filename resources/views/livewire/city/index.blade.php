        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full">
                <div class="w-full grid md:grid-cols-2 gap-2 mb-0 shadow-md dark:bg-gray-800 p-5 bg-white dark:border-gray-700">
                    <div>
                        {{-- Busca --}}
                        <x-custom.input-search id="search" type="text" class="block w-full" text="Busque por nome" model="filters.search" placeholder="" />
                    </div>
                    <div>
                        {{-- Btn nova cidade --}}
                        <x-custom.add-button wire:click.prevent="$emitTo('city.create', 'open')" class="float-right mt-4" title="Nova Cidade" wire:loading.attr="disabled" />
                    </div>
                </div>
                @if (count($items) > 0)
                                    ...
                                    <tr class="text-azul-900 dark:text-gray-400">
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="flex items-center space-x-4 text-sm">
                                                {{-- Editar --}}
                                                <x-custom.edit-button wire:click.prevent="$emitTo('city.edit', 'open', {{ $item->id }})" title="Editar" wire:loading.attr="disabled" />
                                                {{-- Apagar --}}
                                                <x-custom.delete-button wire:click.prevent="$emitTo('city.delete', 'open', {{ $item->id }})" title="Apagar" wire:loading.attr="disabled" />
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
