        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full">
                <div class="w-full grid md:grid-cols-2 gap-2 mb-0 shadow-md dark:bg-gray-800 p-5 bg-white dark:border-gray-700">
                    ...
                    <div>
                        {{-- Btn novo adm --}}
                        <x-custom.add-button wire:click.prevent="$emitTo('area.create', 'open')" class="float-right mt-4" title="Nova Área de Atuação" wire:loading.attr="disabled" />
                    </div>
                </div>
            </div>
            @if (count($items) > 0)
                    ...
                    <table class="w-full table-auto">
                        <thead>
                            <tr
                                ...
                                <th class="px-3 py-2">Relações</th>
                                ...
                                <tr class="text-azul-900 dark:text-gray-400">
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="flex items-center text-sm">
                                            @if (strlen($item->icon) > 3)
                                                <img src="{{ url('storage/areas/' . $item->icon) }}" class="w-5 mr-3 dark:svg_fix">
                                            @endif
                                    ...
                                    <td class="px-3 py-2 text-sm">
                                        @if (strlen($item->image) > 3)
                                            <a href="{{ url('storage/areas/' . $item->image) }}" target="_blank">
                                                <img src="{{ url('storage/areas/' . $item->image) }}" class="w-24">
                                            </a>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="flex items-center space-x-4 text-sm">
                                            {{-- Relacionados
                                            <x-custom.related-button wire:click.prevent="$emitTo('area.related', 'open', {{ $item->id }})" title="Áreas relacionadas" wire:loading.attr="disabled" /> --}}
                                            {{-- Editar --}}
                                            <x-custom.edit-button wire:click.prevent="$emitTo('area.edit', 'open', {{ $item->id }})" title="Editar" wire:loading.attr="disabled" />
                                            {{-- Apagar --}}
                                            <x-custom.delete-button wire:click.prevent="$emitTo('area.delete', 'open', {{ $item->id }})" title="Apagar" wire:loading.attr="disabled" />
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
