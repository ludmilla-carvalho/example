        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full">
                <div class="w-full grid md:grid-cols-2 gap-2 mb-0 shadow-md dark:bg-gray-800 p-5 bg-white dark:border-gray-700">
                    ...
                    <div>
                        {{-- Btn novo  --}}
                        <x-custom.add-button wire:click.prevent="$emitTo('partner.create', 'open')" class="float-right mt-4" title="Novo Parceiro" wire:loading.attr="disabled" />
                    </div>
                </div>
            </div>
            @if (count($items) > 0)
                                ...
                                <tr class="text-azul-900 dark:text-gray-400">
                                    ...
                                    <td class="px-3 py-2 text-sm">
                                        @if (strlen($item->image) > 3)
                                            <a href="{{ url('storage/partners/' . $item->image) }}" target="_blank">
                                                <img src="{{ url('storage/partners/' . $item->image) }}" class="w-24">
                                            </a>
                                        @endif
                                    </td>
                                    <td class="px-3 py-2">
                                        <button wire:click.prevent="$emitTo('partner.edit', 'tooglePublish', {{ $item->id }})" title="Publicar" wire:loading.attr="disabled" />
                                        @if ($item->publish == 1)
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="flex items-center space-x-4 text-sm">
                                           <x-custom.edit-button wire:click.prevent="$emitTo('partner.edit', 'open', {{ $item->id }})" title="Editar" wire:loading.attr="disabled" />
                                            {{-- Apagar --}}
                                            <x-custom.delete-button wire:click.prevent="$emitTo('partner.delete', 'open', {{ $item->id }})" title="Apagar" wire:loading.attr="disabled" />
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
            ...
            <livewire:partner.create>

            <livewire:partner.edit>

            <livewire:partner.delete>
        </div>
    </div>
</div>
