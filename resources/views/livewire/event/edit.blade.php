<div>
    <x-custom.dialog-modal wire:model="open">
         ...
         <x-slot name="title">
             <div class="inline-flex items-center pt-5" id="editEvent">
         ...
         <x-slot name="content">     
             <div class="md:grid md:grid-cols-2 gap-4 pt-1 pb-6" x-data>
                 ...
                 <x-events.form action="edit" :image="$image" :event="$event" :speakers="$speakers" :selSpeakers="$selSpeakers" :selSchedules="$selSchedules" :iteration="$iteration"></x-events.form>
         ...
         <x-slot name="footer">
             ...
             @endif
             <x-custom.secondary-button wire:click="$toggle('open')" x-on:click.prevent="location.href='#editEvent'" wire:loading.attr="disabled">
                 Cancelar
             </x-custom.secondary-button>
     
             <x-custom.button class="ml-2" wire:click="save" x-on:click.prevent="location.href='#editEvent'" wire:loading.attr="disabled">
                 Salvar
             </x-custom.button>
         </x-slot>
