        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full">
                <div class="w-full grid md:grid-cols-2 gap-2 mb-0 shadow-md dark:bg-gray-800 p-5 bg-white dark:border-gray-700">
                    ...
                    <div>
                        {{-- Btn nova experiência --}}
                        <x-custom.add-button wire:click.prevent="$emitTo('experience.create', 'open')" class="float-right mt-4" title="Nova Experiência" wire:loading.attr="disabled" />
                    </div>
                </div>
                @if (count($items) > 0)
                                    ...
                                    <tr class="text-azul-900 dark:text-gray-400">
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="flex items-center space-x-4 text-sm">
                                                {{-- Editar --}}
                                                <x-custom.edit-button wire:click.prevent="$emitTo('experience.edit', 'open', {{ $item->id }})" title="Editar" wire:loading.attr="disabled" />
                                                {{-- Apagar --}}
                                                <x-custom.delete-button wire:click.prevent="$emitTo('experience.delete', 'open', {{ $item->id }})" title="Apagar" wire:loading.attr="disabled" />
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <livewire:experience.create>

                <livewire:experience.edit>

                <livewire:experience.delete>
            </div>
        </div>
    </div>
</div>
