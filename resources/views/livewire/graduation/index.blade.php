        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full">
                <div class="w-full grid md:grid-cols-2 gap-2 mb-0 shadow-md dark:bg-gray-800 p-5 bg-white dark:border-gray-700">
                    ...
                    <div>
                        {{-- Btn nova Formação Acadêmica --}}
                        <x-custom.add-button wire:click.prevent="$emitTo('graduation.create', 'open')" class="float-right mt-4" title="Nova Formação Acadêmica" wire:loading.attr="disabled" />
                    </div>
                </div>
                @if (count($items) > 0)
                        ...
                        <table class="w-full table-auto">
                            <thead>
                                <tr
                                    ...
                                    {{-- <th class="px-3 py-2">Título (en)</th> --}}
                                    ...
                                    <tr class="text-azul-900 dark:text-gray-400">
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="flex items-center space-x-4 text-sm">
                                                {{-- Editar --}}
                                                <x-custom.edit-button wire:click.prevent="$emitTo('graduation.edit', 'open', {{ $item->id }})" title="Editar" wire:loading.attr="disabled" />
                                                {{-- Apagar --}}
                                                <x-custom.delete-button wire:click.prevent="$emitTo('graduation.delete', 'open', {{ $item->id }})" title="Apagar" wire:loading.attr="disabled" />
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <livewire:graduation.create>

                <livewire:graduation.edit>

                <livewire:graduation.delete>
            </div>
        </div>
    </div>
</div>
