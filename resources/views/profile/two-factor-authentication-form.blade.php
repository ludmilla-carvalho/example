<x-action-section>
    <x-slot name="content">
        ...
        <div class="mt-5">
            @if (! $this->enabled)
            <x-custom.button type="button" wire:click="enableTwoFactorAuthentication" wire:loading.attr="disabled">
                {{ __('Enable') }}
            </x-custom.button>
            @else
            @if ($showingRecoveryCodes)
            <x-custom.secondary-button class="mr-3" wire:click="regenerateRecoveryCodes">
                {{ __('Regenerate Recovery Codes') }}
            </x-custom.secondary-button>
            @else
            <x-custom.secondary-button class="mr-3" wire:click="$toggle('showingRecoveryCodes')">
